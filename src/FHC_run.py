...
class RHSCFedTraining(ABC):
    def __init__(self, loger, num_clients, num_abnormal_clients, num_normal_clients, num_rounds: int,
                 ...
                 save_path=None, rep_model='RNNAE', eval_device=torch.device("cuda:0"), personalization=True, global_init_steps=0):
    ...
    def round(self, rnd, trainset, validset, host_list, is_aug):
        ...
        if self.global_init_steps < rnd:
            ...
            validset = np.array(validset,dtype=object)
            host_list = np.array(host_list,dtype=str)
...
if __name__ == '__main__':
    ...
    loger = make_logger(f'./{file_name}.log')
